/**
 * CLIENT-SIDE FALLBACK WEATHER SIMULATOR
 * --------------------------------------
 * Deterministic, seasonal climate approximation used ONLY when the live
 * backend (/api) is unreachable — e.g. a static GitHub Pages build where no
 * Express server is running. Every value is explicitly labelled
 * source:"simulated" / dataMode:"fallback" so it can never be confused with
 * live IMD data. This is a TypeScript mirror of server/src/demo/simulator.js
 * so the demo behaves identically with or without a backend.
 */
...
const COASTAL: Record<string, number> = { mumbai: 1, chennai: 1, kolkata: 1 };
...
function noise(seed: string, salt: string): number {
...
function normalsForDate(city: string, date: Date) {
  const m = CLIMATE[city] || CLIMATE['new-delhi'];
  const idx = date.getMonth();
  const cur = m[idx];
  const nxt = m[(idx + 1) % 12];
  const daysInMonth = new Date(date.getFullYear(), idx + 1, 0).getDate();
  const t = Math.min(1, (date.getDate() - 1) / daysInMonth);
  return {
...
function conditionFromRain(rainMm: number): string {
...
function locFor(id: string) {
  return LOCATIONS.find((l) => l.id === id) ?? LOCATIONS[0];
}

function simulateCurrent(city: string, date: Date) {
  const loc = locFor(city);
  const n = normalsForDate(loc.id, date);
  const seed = `${loc.id}|${dayKey(date)}`;
  ...
  let condition = rainfall === 0 ? '' : conditionFromRain(rainfall);
  if (rainfall === 0) {
  ...
  const coastal = COASTAL[loc.id] === 1;
...
function simulateForecast(city: string, days: number, date: Date): ForecastDay[] {
  const loc = locFor(city);
  const seed = `fc|${loc.id}`;
  const out: ForecastDay[] = [];
  for (let i = 0; i < days; i += 1) {
    const d = new Date(date.getFullYear(), date.getMonth(), date.getDate() + i);
    const n = normalsForDate(loc.id, d);
    ...
    out.push({
      ...
      tMax: Math.round((n.tMax + r * 1.8) * 10) / 10,
      tMin: Math.round((n.tMin + noise(seed, `n${i}`) * 1.4) * 10) / 10,
      ...
      windSpeed: Math.round((COASTAL[loc.id] ? 18 : 11) * (0.6 + Math.abs(noise(seed, `w${i}`))))
    });
  }
  return out;
}

function simulateHourly(city: string, date: Date): HourlyPoint[] {
  const loc = locFor(city);
  const seed = `hr|${loc.id}`;
  const n = normalsForDate(loc.id, date);
...
function simulateWarnings(city: string, date: Date): Warning[] {
  const loc = locFor(city);
  const n = normalsForDate(loc.id, date);
  const r = noise(`warn|${loc.id}|${dayKey(date)}`, 'sev');
...
function simulateNowcast(city: string, date: Date): NowcastItem[] {
  const loc = locFor(city);
  const warnings = simulateWarnings(loc.id, date);
  const n = normalsForDate(loc.id, date);
...
function simulateRainfall(city: string, date: Date): RainfallRow[] {
  const loc = locFor(city);
  const n = normalsForDate(loc.id, date);
  const base = n.rain * (0.5 + Math.abs(noise(`rf|${loc.id}|${dayKey(date)}`, 'x')));
  return [
    {
      district: loc.name,
      state: loc.state,
...
/**
 * Full normalized WeatherEnvelope from the deterministic simulator.
 * Always marked dataMode:"fallback" / source:"simulated" — never confused
 * with live IMD data. Returns null for unknown locations.
 */
export function simulateWeatherEnvelope(locationId: string, demo: boolean, now = new Date()): WeatherEnvelope | null {
  const loc = locFor(locationId);
  if (!loc) return null;
  const current = simulateCurrent(loc.id, now);
  const forecast = simulateForecast(loc.id, 7, now);
  const hourly = simulateHourly(loc.id, now);
  const warnings = simulateWarnings(loc.id, now);
  const nowcast = simulateNowcast(loc.id, now);
  const rainfall = simulateRainfall(loc.id, now);
  ...
  return {
    ...
    current: {
      ...current,
      source: 'simulated',
      sourceMeta: { live: false, via: 'client-fallback-simulator', provider: 'India Meteorological Department', label: 'SIMULATED' }
    },
    hourly,
    forecast,
    warnings,
    ...
    meta: {
      dataMode: 'fallback',
      dataSourceLabel: demo ? 'DEMO FALLBACK DATA' : 'SIMULATED (backend unreachable)',
      imdUnavailable: true,
      lastUpdated: now.toISOString(),
      sources: { current: 'simulated', forecast: 'simulated', warnings: 'simulated', nowcast: 'simulated', rainfall: 'simulated' },
      liveStatus: {
        imdV1: 'unreachable',
